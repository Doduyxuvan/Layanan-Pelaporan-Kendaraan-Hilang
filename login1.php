...
require "+koneksi1.php"; // memanggil file koneksi
?>
<!DOCTYPE html>
<html>
<head>
...
if(isset($_POST['login'])) {
    ...
    if($cek > 0) {
        $data = $query->fetch();
        if($user === 'polisi') { // Periksa apakah username adalah 'polisi'
            $_SESSION['iduser'] = $data['id'];
            $_SESSION['username'] = $data['username'];
            echo "<script>alert('Selamat. Login berhasil :)'); window.location='home1.php';</script>";
        } else {
            echo "<script>alert('Login gagal. Hanya pengguna dengan username \'polisi\' yang diperbolehkan!');</script>";
        }
    } else {
