...
// Include file check_access.php untuk memeriksa hak akses
...
$query = "SELECT * FROM curanmor";
if ($search) {
    $query .= " WHERE lapor LIKE '%$search%' 
                OR pelapor LIKE '%$search%' 
                OR uraian_singkat LIKE '%$search%' 
                OR tkp LIKE '%$search%' 
                OR kasus LIKE '%$search%' 
                OR tersangka LIKE '%$search%' 
...
<div id="main">
    ...
    <div class="container">
        <a href="add.html" class="btn btn-primary mb-3">Tambah Data Baru</a>
        <h3>Data Curanmor</h3>
        <form action="" method="get">
            <input type="text" name="search" placeholder="Cari data curanmor..." value="<?php echo htmlspecialchars($search); ?>">
            <input type="submit" value="Search" style="padding: 1px 6px; font-size: 14px; width: auto;">
        </form>
                ...
                <tbody>
                    ...
                    while ($res = mysqli_fetch_array($result)) { ?>
                        <tr>
                            ...
                            <td><?php echo $res['lapor']; ?></td>
                            <td><?php echo $res['tkp']; ?></td>
                            <td><?php echo $res['kasus']; ?></td>
                            <td><?php echo $res['pelapor']; ?></td>
                            <td><?php echo $res['tersangka']; ?></td>
                            <td><?php echo $res['uraian_singkat']; ?></td>
                            <td><?php echo $res['keterangan']; ?></td> <!-- Kolom keterangan -->
                            <td><img src='image/<?php echo $res['gambar']; ?>' width="100" /></td> <!-- Kolom gambar barang bukti -->
                            <td>
                                <a href="edit.php?id=<?php echo $res['id']; ?>" class="btn btn-warning btn-sm">Edit</a>
                                <a href="delete.php?id=<?php echo $res['id']; ?>" class="btn btn-danger btn-sm" onClick="return confirm('Kamu yakin untuk delete ini?')">Delete</a>
                            </td> <!-- Kolom Action -->
                        </tr>
                    <?php } ?>
                </tbody>
